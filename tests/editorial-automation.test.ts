...
test("classifies editorial failures without exposing provider details", () => {
  assert.equal(
    classifyEditorialFailure(new Error("NVIDIA 429 rate limit")),
    "ai_generation_failed",
  );
  assert.equal(
    classifyEditorialFailure(new Error("invalid_generated_json")),
    "schema_validation_failed",
  );
  assert.equal(
    classifyEditorialFailure(new Error("Firestore permission-denied")),
    "firebase_write_failed",
  );
  assert.equal(
    classifyEditorialFailure(
      Object.assign(new Error("deadline exceeded"), { code: 504 }),
    ),
    "timeout",
  );
  assert.equal(
    safeEditorialFailureMessage("firebase_write_failed"),
    "The editorial queue could not be saved.",
  );
});

test("removes undefined optional fields before Firestore serialization", () => {
  const safe = firestoreSafeIngestStory({
    ...story,
    imageUrl: undefined,
    location: undefined,
  });
...
const story: IngestStory = {
  ...
  domain: "Technology",
  summary:
    "A student engineering team has demonstrated a lower-cost satellite communications platform after completing tests at its university laboratory.",
  keyFacts: [
...
const generated: Pick<Article, "quick_brief" | "full_article"> = {
  quick_brief: {
    category: "Technology",
    headline:
      "Indian students demonstrate lower-cost satellite communications platform",
    quick_summary:
      "A university engineering team has demonstrated a lower-cost satellite communications platform in controlled laboratory testing, aiming to make communications hardware more accessible without changing the facts reported by the original source.",
  ...
  full_article: {
    ...
    in_20_seconds:
      "A university engineering team completed a controlled demonstration of its communications platform.",
    what_happened:
      "A university student engineering team has demonstrated a satellite communications platform developed to explore lower-cost hardware. According to the supplied report, the team completed a controlled laboratory test this week. The demonstration focused on the prototype described by the students and did not establish commercial availability or performance beyond the reported test.",
    why_this_matters:
      "Satellite communications projects can give engineering students experience across hardware, software, radio systems and testing. A prototype focused on lowering hardware costs may also help students examine practical constraints that are often hidden in classroom exercises, while the reported laboratory result remains an early technical step rather than a commercial launch.",
    bigger_picture:
      "Student engineering teams frequently use working prototypes to turn classroom concepts into systems that can be tested, measured and improved. This project sits within that educational pattern, with any broader impact dependent on later evidence and development.",
    key_stats: [],
    explore_sections: [
      {
        ...
        content:
          "The supplied reporting describes a satellite communications platform built by a university engineering team. Its stated focus is reducing the cost of communications hardware while giving the team a system they can evaluate through controlled testing and iteration in a laboratory setting.",
        items: [],
      },
      {
        ...
        content:
          "The team completed a controlled laboratory demonstration this week. The report does not claim commercial deployment, independent certification or performance outside the laboratory, so the result should be understood as an early prototype milestone that can guide later engineering work.",
        items: [],
      },
    ],
    takeaways: [
...
test("sample ingestion reaches review and publishes the canonical schema", async () => {
  ...
  const published = await service.publish(ready.id, {
    uid: "editor",
    name: "Editor",
...
test("malformed payload is rejected before queue creation", async () => {
  ...
  await assert.rejects(
    () => service.ingest({ title: "Bad" }),
    (error: EditorialError) => error.code === "invalid_payload",
  );
...
test("async ingestion returns before a slow generation and resolves later", async () => {
  ...
  let resolveGeneration!: (
    value: Pick<Article, "quick_brief" | "full_article">,
  ) => void;
  const pending = new Promise<Pick<Article, "quick_brief" | "full_article">>(
    (resolve) => {
  ...
  assert.ok(
    ...
    "queue creation must not wait for generation",
  );
...
test("exact and likely duplicates stop before generation", async () => {
  ...
  const exact = await service.ingest({
    ...story,
    sourceUrl:
      "https://www.example.com/news/student-satellite?utm_medium=email",
  });
  assert.equal(exact.status, "discovered");
  assert.equal(exact.duplicate?.kind, "exact");
  const likely = await service.ingest({
    ...story,
    sourceUrl: "https://another.example/report",
...
test("NVIDIA formatter failure and invalid output are isolated as failed items", async () => {
  ...
  const failing = new EditorialService(failingStore, async () => {
  ...
  const invalidItem = await invalid.ingest({
    ...story,
