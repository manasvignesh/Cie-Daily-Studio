...
export type EditorialStore = {
  ...
  claim?: (
    id: string,
    staleAfterMs: number,
  ) => Promise<{ claimed: boolean; item: EditorialQueueItem | null }>;
  publish(queueId: string, post: Record<string, unknown>): Promise<string>;
};
 ...
 */
export function classifyEditorialFailure(
  error: unknown,
): EditorialFailureCategory {
  ...
  if (
    /timeout|timed.?out|deadline|aborted|aborterror/.test(
      `${code} ${name} ${message}`,
    )
  ) {
    return "timeout";
  ...
  if (
    /invalid_generated_json|generated_schema_missing|failed validation|schema_validation/.test(
      message,
    )
  ) {
  ...
  if (
    /ai_not_configured|nvidia|gemini|openai|empty_ai_response|generation_failed|chat\.completions/.test(
      `${code} ${name} ${message}`,
    )
  ) {
    return /ai_not_configured|not configured|missing.*(key|secret)|api key/.test(
      message,
    )
      ? "configuration_missing"
      : "ai_generation_failed";
  ...
  if (
    /firebase|firestore|google.?cloud|permission-denied|failed-precondition|unavailable|service account|default credentials/.test(
      `${code} ${name} ${message}`,
    )
  ) {
...
export function safeEditorialFailureMessage(
...
export function validateIngestStory(
  ...
  if (
    !allowedDomains.some(
      (domain) => domain.toLowerCase() === story.domain.toLowerCase(),
    )
  ) {
    errors.push(`domain must be one of: ${allowedDomains.join(", ")}`);
  } else {
    story.domain = allowedDomains.find(
      (domain) => domain.toLowerCase() === story.domain.toLowerCase(),
    )!;
  }
  ...
  if (story.keyFacts.length < 2)
    errors.push("keyFacts must contain at least 2 facts");
...
export function findDuplicate(
  ...
  for (const item of existing) {
    ...
    if (
      ...
      Number.isFinite(otherPublished) &&
...
export class EditorialService {
  ...
  async regenerate(id: string) {
    ...
    if (item.status === "published" || item.status === "rejected") {
      throw new EditorialError(
        "invalid_status",
        "Published or rejected items cannot be regenerated.",
        409,
      );
    }
    await this.store.update(id, {
  ...
  async edit(id: string, generatedArticle: Article) {
    ...
    if (item.status === "published" || item.status === "rejected") {
      throw new EditorialError(
        "invalid_status",
        "Published or rejected items cannot be edited.",
        409,
      );
    }
    if (!generatedArticle?.quick_brief || !generatedArticle?.full_article) {
      throw new EditorialError(
        "invalid_article",
        "Both Quick Brief and Full Story are required.",
        422,
      );
    }
  ...
  async reject(id: string) {
    ...
    if (item.status === "published") {
      throw new EditorialError(
        "invalid_status",
        "Published items cannot be rejected.",
        409,
      );
    }
